...
for file_R1 in $INPUT_30X/*_R1.fastq.gz; do
  ...
  pcne -c $CHROMOSOME -p $PLASMID -r ${file_R1} -R ${file_R2} -t 4  --plot -b --gc-correction --gc-plot $OUTPUT_30X/${sample_name}.png -Q 10 -F 2308 -o $OUTPUT_30X/${sample_name}_scg_filter_gc
done  
